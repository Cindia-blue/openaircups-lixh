...
bool pgw_config::is_dotted_apn_handled(const string& apn, const pdn_type_t& pdn_type)
{
  ...
  for (int i = 0; i < pgw_cfg.num_apn; i++) {
    Logger::pgwc_app().trace( " ***** is_dotted_apn_handled() i: %d, apn: %s, pgw_cfg.apn[i].apn_label: %s, pgw_cfg.apn[i].apn %s", i, apn.c_str(), pgw_cfg.apn[i].apn_label.c_str(), pgw_cfg.apn[i].apn.c_str());
