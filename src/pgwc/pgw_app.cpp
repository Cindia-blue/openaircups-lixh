...
int pgw_app::apply_config (const pgw_config& cfg)
{
  ...
  Logger::pgwc_app().info(" ***** apply_config: cfg.num_apn: %d", cfg.num_apn);

  for (int ia = 0; ia < cfg.num_apn; ia++) {
    Logger::pgwc_app().info(" ***** apply_config: pool_id_iv4: %d, pool_id_iv6: %d, apn_label: %s", cfg.apn[ia].pool_id_iv4, cfg.apn[ia].pool_id_iv6, cfg.apn[ia].apn_label.c_str());
...
void pgw_app::handle_itti_msg (std::shared_ptr<itti_s5s8_create_session_request> scsreq)
{
  ...
  Logger::pgwc_app().trace( " ***** access_point_name:  %s", csreq->gtp_ies.apn.access_point_name.c_str());
  if (csreq->gtp_ies.apn.access_point_name[0] < 32) {
	  Logger::pgwc_app().trace( " ***** Changing access_point_name from %s", csreq->gtp_ies.apn.access_point_name.c_str());
	  csreq->gtp_ies.apn.access_point_name.erase(0, 1);
	  Logger::pgwc_app().trace( " ***** Changing access_point_name to %s", csreq->gtp_ies.apn.access_point_name.c_str());
  }
